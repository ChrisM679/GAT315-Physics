...
*
*   Copyright (c) 2022 Munson. All Rights Reserved.
*
...
GuiPhysicsState InitGuiPhysics(void)
{
    ...
    state.BodySizeValue = 0.2f;       // world units (~20cm radius), was 5.0 pixels
    state.BodyGravityValue = 1.0f;
    state.BodyRestitutionValue = 0.5f;
    state.BodyDampingValue = 0.1f;
    state.BodyTypeEditMode = false;
    state.BodyTypeActive = 2;         // 0=Static, 1=Kinematic, 2=Dynamic
    ...
    state.EffectorSizeValue = 2.0f;   // world units radius for effectors
    state.EffectorForceValue = 10.0f; // reasonable world-scale force
    state.EffectorAngleValue = 0.0f;
    state.SimulateActive = true;
    state.GravityValue = -9.81f;      // negative = down in world space (camera flips Y)
    state.FPSValue = 60.0f;
    state.BodyVelocityValue = 5.0f;   // world units/sec, was 40 pixels/sec
...
void GuiPhysics(GuiPhysicsState* state)
{
    ...
    if (state->PhysicsPanelActive)
    {
        ...
        GuiSliderBar((Rectangle) { state->anchor02.x + 80, state->anchor02.y + 88, 184, 16 }, "SIZE", GUI_TEXT(state->BodySizeValue), & state->BodySizeValue, 0.05f, 2.0f);
        ...
        GuiSliderBar((Rectangle) { state->anchor02.x + 80, state->anchor02.y + 208, 184, 16 }, "VELOCITY", GUI_TEXT(state->BodyVelocityValue), & state->BodyVelocityValue, 0, 20);
        ...
        if (GuiDropdownBox((Rectangle) { state->anchor02.x + 32, state->anchor02.y + 48, 240, 24 }, "STATIC;KINEMATIC;DYNAMIC", & state->BodyTypeActive, state->BodyTypeEditMode)) state->BodyTypeEditMode = !state->BodyTypeEditMode;
